...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        heap = []
        heapq.heappush(heap, (0, 0, src)) # cost, step, id

        min_dst_cost = sys.maxsize
        while heap:
            now_cost, now_step, now = heapq.heappop(heap)
            if now == dst:
                min_dst_cost = min(min_dst_cost, now_cost)
            if now_step == k + 1:
                continue
            if min_step[now] <= now_step:
                continue
            ...
            for next, edge_cost in graph[now]:
                ...
                heapq.heappush(heap, (next_cost, next_step, next))
        
        return min_dst_cost if min_dst_cost != sys.maxsize else -1
```
### Time Complexity
* O(k * E * (E + log(k * E)))
