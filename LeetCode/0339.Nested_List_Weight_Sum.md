...
class Solution:
    ...
    def dfs(self, list_now, depth_now):
        for element in list_now:
            if element.isInteger():
                self.depth_sum += element.getInteger() * depth_now
            else:
                self.dfs(element.getList(), depth_now + 1)
```
### Time complexity
* O(n)
### Space complexity
